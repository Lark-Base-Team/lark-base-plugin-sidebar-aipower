...
// 添加停止执行的方法
const stopWorkflow = () => {
  executionStatus.value.shouldStop = true;
  ElMessage.warning('正在停止工作流执行...');
};
...
async function executeWorkflow() {
  ...
  try {
    ...
    for (let i = 0; i < records.length; i++) {
      // 检查是否应该停止
      if (executionStatus.value.shouldStop) {
        throw new Error('工作流执行已手动停止');
      }
    ...
    if (error.message === '工作流执行已手动停止') {
      ElMessage.info('工作流执行已停止');
    } else {
      ElMessage.error(error.message);
    }
  } finally {
    executionStatus.value.running = false;
    executionStatus.value.shouldStop = false;
  }
}
...
onMounted(async () => {
  try {
    // 加载保存的Token
    const savedAiPowerToken = localStorage.getItem('aiPowerToken');
    if (savedAiPowerToken) {
      aiPowerToken.value = savedAiPowerToken;
      await loadWorkflows();
    }
    ...
    if (databaseList.value?.length) {
      try {
        for (const table of databaseList.value) {
          try {
            const currencyFields = await table.getFieldMetaList();
            const hasCurrencyField = currencyFields.some(field => field.type === 28); // 28 是货币类型
            if (hasCurrencyField) {
            ...
            console.warn('获取表格字段失败:', fieldError);
            continue;
          }
        }
        ...
        console.warn('处理数据表失败:', tableError);
        // 如果处理失败，至少选择第一个表
    ...
    setTimeout(() => {
      try {
        base.onDataChange(async (event) => {
          try {
            console.log('数据发生变化:', event);
            const newDatabaseList = await base.getTableMetaList();
            if (newDatabaseList) {
              databaseList.value = newDatabaseList;
              if (databaseId.value && databaseList.value.some(table => table.id === databaseId.value)) {
                await databaseChange();
              } else if (databaseList.value.length > 0) {
                databaseId.value = databaseList.value[0].id;
                await databaseChange();
              }
            }
          } catch (innerError) {
            console.warn('数据变化处理失败:', innerError);
          }
        });
      } catch (listenerError) {
        console.warn('注册数据变化监听器失败:', listenerError);
      }
    }, 2000);
    
  } catch (error) {
    console.error('初始化失败:', error);
    // 只有在真正影响功能的错误时才显示错误提示
    if (error.message !== 'Network Error' && !databaseList.value) {
...
<template>
    ...
    <!-- 在最后一个section-card后添加 -->
    <div class="tutorial-section">
      <el-link 
        type="primary" 
        :underline="false" 
        href="https://ying-dao.feishu.cn/docx/Oq7kd9VRbocTDXx8dYJciuCkncc" 
        target="_blank"
        class="tutorial-link"
      >
        使用教程
